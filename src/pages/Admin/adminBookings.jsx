...
const AdminBookings = () => {
  ...
  const { bookings, error, loading,totalPages } = useSelector((state) => state.admin);
  ...
  const colums = [
    ...
    { header: "Worker name", key: "workerId" , render: (item) => item.workerId?.name || "N/A" },
    { header: "User name", key: "userId", render: (item) => item.userId?.username || "N/A" },
    { header: "Status", key: "workStatus",
    render: (item) => (
      <span className={`px-3 py-1 rounded-lg text-center ${getStatusColor(item.workStatus)}`}>
        {item.workStatus}
      </span>
    )},
  ...
  const actions = (item) => {
    if (item.workStatus === "Cancelled" || item.workStatus === "Completed" || item.workStatus === 'In Progress' || item.workStatus === 'Rejected') {
      return [
        {
          label: "Details",
          onClick: () =>{
      ...
      return [
        {
          label: "Cancel",
          onClick: () => dispatch(toggleCancelBooking({bookingId:item.id})),
          bgColor:'bg-red-500'
        },
        {
          label: "Details",
...
{isModalOpen && selectedBooking && (
  <div
    className={`fixed inset-0 z-50 flex items-center justify-center bg-transparent backdrop-blur-sm transition-all duration-300 ${
      isModalOpen ? "opacity-100 scale-100" : "opacity-0 scale-80 pointer-events-none"
    }`}
   onClick={closeModal}>
    <div className="relative bg-white p-6 rounded-lg shadow-lg w-full max-w-xl">
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="flex flex-col space-y-2">
          <p>
            <strong>Worker Name:</strong> {selectedBooking.workerId?.name || "N/A"}
          </p>
          <p>
            <strong>Worker Phone:</strong> {selectedBooking.workerId?.phone || "N/A"}
          </p>
          <p>
            <strong>Work Status:</strong> {selectedBooking.workStatus}
          </p>
          <p>
            <strong>Reaching Status:</strong> {selectedBooking.reachingStatus}
          </p>
        ...
        <div className="flex flex-col items-start text-left space-y-2">
          <p>
            <strong>User Name:</strong> {selectedBooking.userId?.username || "N/A"}
          </p>
          <p>
            <strong>Location:</strong> {selectedBooking.address?.city || "N/A"}
          </p>
          <p>
            <strong>Booking Type:</strong> {selectedBooking.bookingType || "N/A"}
          </p>
        </div>
      </div>
    </div>
